 ...
 */
/**
 * 实现 按天 删除日志 按指定大小分文件记录
 * 
 * 示例: 最后生成的文件同一天内就会按照 10M来分文件，最多分9999个，如果超过9999就会迭代覆盖。 maxTimeRollBackups
 * 是指超期日期是7天，会自动删除7天前的日志 特殊处理：我要求应用再超期日期这段时间内没有重启过才会删除超期后的日志，
 * 如果重启过，超期时间内的日志都不会自动删除可供分析问题。
 * 
 ...
 * log4j.appender.logfile.File=../logs/XXXXXXXXX.log # Keep three backup files.
 ...
 * log4j.appender.logfile.layout.ConversionPattern=[工程名] %d{yyyy-MM-dd HH:mm:ss}
 * [%c]-[%p] %m%n
 * 
...
public class CompositeRollingAppender extends org.apache.log4j.FileAppender {
	...
	/***
	 * The actual formatted filename that is currently being written to or will
	 * be the file transferred to on roll over (based on staticLogFileName).
	 */
	...
	/***
	 * By default newer files have lower numbers. (countDirection < 0) ie. log.1
	 * is most recent, log.5 is the 5th backup, etc... countDirection > 0 does
	 * the opposite ie. log.1 is the first backup made, log.5 is the 5th backup
	 * made, etc. For infinite backups use countDirection > 0 to reduce rollOver
	 * costs.
	 */
	 ...
	 * 
	 * <p>
	 * The <b>MaxSizeRollBackups</b> option determines how many backup files are
	 * kept before the oldest is erased. This option takes an integer value. If
	 * set to zero, then there will be no backup files and the log file will be
	 * truncated when it reaches <code>MaxFileSize</code>. If a negative number
	 * is supplied then no deletions will be made. Note that this could result
	 * in very slow performance as a large number of files are rolled over
	 * unless {@link #setCountDirection} up is used.
	 * 
	 * <p>
	 * The maximum applys to -each- time based group of files and -not- the
	 * total. Using a daily roll the maximum total files would be (#days run) *
	 * (maxSizeRollBackups)
	 */
	...
	/***
	 * Handles append time behavior for CompositeRollingAppender. This checks if
	 * a roll over either by date (checked first) or time (checked second) is
	 * need and then appends to the file last.
	 */
	protected void subAppend(LoggingEvent event) {
		...
		if (rollDate) {
			...
			if (n >= nextCheck) {
				...
				/*
				 * 这里我故意让程序 根据 curTimeRollBackups 来删除文件 也就是说 程序必须是连续的 正常的(无人为重启)
				 * 的运行到 curTimeRollBackups > maxTimeRollBackups 的时间 才会根据
				 * maxTimeRollBackups 去删除日志 如果中途重启过，那么就会留下 从上次启动至
				 * maxTimeRollBackups 这段时间的日志，这段时间的日志 只能人为删除
				 */
	...
	/**
	 * public void setRollingStyle(String style) { if (style == S_BY_SIZE) {
	 * rollingStyle = BY_SIZE; } else if (style == S_BY_DATE) { rollingStyle =
	 * BY_DATE; } else if (style == S_BY_COMPOSITE) { rollingStyle =
	 * BY_COMPOSITE; } }
	 */
	 ...
	 * . The following is done:<br>
	 * <br>
	 * A) determine curSizeRollBackups<br>
	 * B) determine curTimeRollBackups (not implemented)<br>
	 * C) initiates a roll over if needed for crossing a date boundary since the
	 * last run.
	 */
	 ...
	 * check, scheduledFilename, and calls <code>existingInit</code> to
	 * initialize the current # of backups.
	 */
	...
	/***
	 * Rollover the file(s) to date/time tagged file(s). Opens the new file
	 * (through setFile) and resets curSizeRollBackups.
	 */
	...
	/***
	 * Renames file <code>from</code> to file <code>to</code>. It also checks
	 * for existence of target file and deletes if it does.
	 */
	 ...
	 * If <code>countDirection</code> < 0, then files {<code>File.1</code>, ...,
	 * <code>File.curSizeRollBackups -1</code> are renamed to {
	 * <code>File.2</code>, ..., <code>File.curSizeRollBackups</code> .
	 * Moreover, <code>File</code> is renamed <code>File.1</code> and closed.<br>
	 * 
	 * A new file is created to receive further log output.
	 * 
	 * <p>
	 * If <code>maxSizeRollBackups</code> is equal to zero, then the
	 * <code>File</code> is truncated with no backup files created.
	 * 
	 * <p>
	 * If <code>maxSizeRollBackups</code> < 0, then <code>File</code> is renamed
	 * if needed and no files are deleted.
	 */
	...
	private static String getDateString(Date date, int pos) {
		...
		return dateFormat.format(addDays(date, pos));
	}
